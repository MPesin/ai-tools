...
1. **Read** every `docs/agents/.staging/*.md`.
...
4. **Write final files** exactly per the output spec: docs/agents/
...
7. **Write manifest.json** (new stamps: `git rev-parse HEAD`, per-area stamps,
   block checksums via `shasum -a 256`), then delete `docs/agents/.staging/`.
