...
2. Propose domains. Sources, in order of preference: an existing repo-guide
   ...
   domains, with their paths); otherwise the source directories one level
   below the source root (`src/*`, or the top level when there is no
   `src/`), merged to 3-8 domains. Always add a `build` domain (build files, CI,
...
4. **Offer seeding** (never automatic): dispatch `decision-log:decision-miner`
   ...
   candidates are rendered as full records (ids assigned, `status:
   proposed`, `Rejected: unknown (seeded — fill in before accepting)` when
   the miner had none) and shown again exactly as they will be written;
   write on yes. A candidate the miner left `unassigned` is offered a domain
   from the table or skipped — never written without one. Accept them now
   or later with `decide accept D-nnn`. The miner's candidates are claims
   from history, not facts — never mark them `accepted` on the miner's word.
...
`decide accept D-nnn`: show the proposed record; if `Rejected:` is
`unknown`, ask for it (or an explicit "none considered because"); on yes,
rewrite line 2's status to `accepted` and the `Rejected:` line — the only
edits ever made inside a record body — and recommend the commit
`decisions: accept D-nnn`.
...
3. Gates — decline with the reason when one fails:
   ...
   - **Not a duplicate.** `grep -il "<subject>" references/*.md` names
     candidate files; read the matching records in them. If one answers the
     same question, offer supersede (new record + status line on the old
     one) instead of a parallel record.
...
2. Answer in ≤30 lines: for each relevant record, its id, title, one-line
   ...
   question. Currently accepted records (last `status:` line) first;
   superseded ones only when the question is about history or when they
   explain a rejected direction; proposed ones marked as such.
